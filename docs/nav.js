...
(function () {
  ...
  // Modules 1–7 at the top level; each module may carry `projects` (rendered
  // indented under it in the top rail). Pages with explicit <section id> →
  // {t: <label>, id: <id>}; pages without ids → {t: <EXACT h2 text>} (slugged
  // the same way the page does at runtime, so the anchor still resolves).
  const OUTLINE = [
    ...
    { href: "02-1-foundations.html", num: "2", label: "Foundations", sections: [
      ...
      { href: "02-2-chat-app.html", label: "Project: chat-app", sections: [
    ...
    { href: "03A-tools.html", num: "3", label: "Tools & Structure", sections: [
      ...
      { href: "03B-extractor.html", label: "Project: extractor", sections: [
    ...
    { href: "04A-context.html", num: "4", label: "Context / RAG", sections: [
      ...
      { href: "04B-rag.html", label: "Project: RAG", sections: [
    ...
    { href: "05A-agents.html", num: "5", label: "Agents", sections: [
      ...
      { href: "05B-finetune.html", label: "Project: fine-tuning", sections: [
        { t: "5B.1 · What it builds", id: "what" },
        { t: "5B.2 · 왜 어려운가 — 숨은 하우스룰", id: "hard" },
        { t: "5B.3 · 신경망이 분류하는 법", id: "primer" },
        { t: "5B.4 · 파일 지도 & 역할", id: "scaffold" },
        { t: "5B.5 · 세 가지 방법", id: "stages" },
        { t: "5B.6 · 흐름 — stage 2 파이프라인 ★", id: "flow" },
        { t: "5B.7 · Frozen embedder + trained head", id: "hood" },
        { t: "5B.8 · 코드로 보기", id: "code" },
        { t: "5B.9 · Build & run locally", id: "local" },
        { t: "5B.10 · The key idea", id: "key" },
      ] },
      { href: "05C-vla.html", label: "Project: VLA", sections: [
        ...
        { t: "5C.5 · main() 호출 순서 ★", id: "flow" },
        { t: "5C.6 · 먼저 실행: check_setup.py", id: "check" },
        { t: "5C.7 · build_observation", id: "buildobs" },
        { t: "5C.8 · The rollout loop", id: "rollout" },
        { t: "5C.9 · Embodiment & 정직한 한계", id: "embodiment" },
        { t: "5C.10 · Going further", id: "further" },
        { t: "5C.11 · Build & run locally", id: "local" },
        { t: "5C.12 · The key idea", id: "key" },
      ] },
      { href: "05D-agent.html", label: "Project: data agent", sections: [
        { t: "5D.1 · What it builds", id: "what" },
        { t: "5D.2 · Scaffold — 파일 5개", id: "scaffold" },
        { t: "5D.3 · 흐름 — the loop is the engine", id: "how" },
        { t: "5D.4 · The database", id: "schema" },
        { t: "5D.5 · 코드 — 세 조각으로 읽기", id: "code" },
        { t: "5D.6 · Run it live", id: "run" },
        { t: "5D.7 · Build & run locally", id: "local" },
        { t: "5D.8 · The key idea", id: "key" },
      ] },
    ] },
  ...
  // ── build the rail in two parts: modules (with indented projects) on top,
  //    then the CURRENT page's sections in detail below (with scroll-spy) ──
  const nav = document.createElement("nav");
  nav.className = "rail-outline";
  const linkBySectionId = new Map(); // current-page section id → its <a> (scroll-spy)

  // Part 1 (top): modules 1–7, each followed by its indented project links.
  ...
  for (const mod of OUTLINE) {
    ...
    for (const proj of mod.projects || []) {
      ...
      projA.className = "rail-proj";
      projA.href = "./" + proj.href;
      projA.textContent = proj.label;
      if (proj.href === here) projA.classList.add("current-mod");
      mods.appendChild(projA);
    }
  }
  ...
  if (!curEntry) {
    for (const mod of OUTLINE) {
      ...
      if (p) { curEntry = p; curTitle = "이 페이지 · " + p.label; break; }
    }
  }
  if (curEntry && curEntry.sections.length) {
    ...
    for (const s of curEntry.sections) {
