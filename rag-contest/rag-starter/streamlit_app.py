...
# Shown live in the agentic column so viewers can see WHAT the loop does while it runs.
AGENTIC_LOOP_SRC = '''for _turn in range(MAX_TURNS):              # a few rounds
    resp = claude.create(tools=[search_cfr], messages=msgs)
    if resp.stop_reason != "tool_use":         # model is satisfied → final answer
        return resp.text
    for call in resp.tool_calls:               # the model asked to search
        if searches < BUDGET:                  # BUDGET = 3
            hits = select_context(retrieve(call.query))
            registry += hits; searches += 1    # remember for [n] citations
        else:
            hits = "budget reached — answer now"
        msgs += tool_result(hits)              # feed results back, loop again'''
    ...
    with st.chat_message("assistant", avatar=AVATARS["assistant"]):
        ...
        with col_ag:
            ...
            with st.status("Running the agentic search loop…", expanded=True) as status:
                st.caption("It repeats **call `search_cfr` → read hits → search again** until it "
                           "has every part (budget 3), then writes the answer:")
                st.code(AGENTIC_LOOP_SRC, language="python")
