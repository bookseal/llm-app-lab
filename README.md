...
├── extract-starter/     Module 3 스타터 — 구조화 추출(forced tool_use + Pydantic)
├── instructor-extractor/ Module 3 확장 예제 — 강사 구인 텍스트 추출 웹앱
...
## 직접 만든 예제 — instructor-extractor (Module 3)

Module 3(Tools & Structure)의 `extract-starter`를 **실제 웹앱으로 확장**한 예제입니다.
강사 구인 글(이메일·카톡 복붙)을 붙여넣으면 → 구조화 필드로 추출하고, **빈 필드는 ⚠️ 누락**으로
표시하며, 누락 정보를 **되묻는 follow-up 이메일 초안**까지 Claude가 작성해 줍니다.

![instructor-extractor 데모](docs/assets/instructor-extractor.png)

- **두 방향의 LLM 사용을 한 화면에서** — 추출(글→데이터, `tool_choice` 강제) + 생성(데이터→글, 자유 텍스트).
- **점진적 개발** — UI 목업 먼저 → 가짜 데이터 → 진짜 Claude 추출 → 필드 한 칸씩 확장.
- 스택: Flask 단일 서버. 실행은 [instructor-extractor/README.md](instructor-extractor/README.md) 참고.
